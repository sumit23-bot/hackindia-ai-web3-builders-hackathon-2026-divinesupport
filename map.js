...
window.locateUserAndCenter = function() {
  const statusText = document.getElementById('radar-status-text');
  if (statusText) statusText.textContent = "Acquiring live hardware GPS...";
  ...
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        localStorage.setItem('foodloop_user_coords', JSON.stringify(userGPS));
        if (mapInstance) {
          mapInstance.flyTo([userGPS.lat, userGPS.lon], 13, { animate: true, duration: 1.2 });
          plotUserLocationMarker();
        }
        if (statusText) statusText.textContent = `Active: ${userGPS.lat.toFixed(4)}, ${userGPS.lon.toFixed(4)}`;
        renderMapMarkersAndList();
      },
      () => {
        if (mapInstance) {
